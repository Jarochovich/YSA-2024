...
namespace GRB
{
	...
	Greibach greibach(NS('S'), TS('$'), 12,
		...
		Rule(NS('N'), GRB_ERROR_SERIES + 1, 	// Ошибочный оператор
			5,	// tY;N |  i=E;N | cU;N | ~K{N}N | пустой переход
			Rule::Chain(4, TS('t'), NS('Y'), TS(';'), NS('N')),
			Rule::Chain(5, TS('i'), TS('='), NS('E'), TS(';'), NS('N')),
			Rule::Chain(4, TS('c'), NS('E'), TS(';'), NS('N')),
		...
		Rule(NS('Y'), GRB_ERROR_SERIES + 6, 	// Ошибка при определении переменной
			2,	// vi || vi = L
		...
		Rule(NS('K'), GRB_ERROR_SERIES + 7, 	// Ошибка при определении условия перехода
			2,	// (U<U) | (U>U)
			Rule::Chain(5, TS('('), NS('U'), TS('<'), NS('U'), TS(')')),
			Rule::Chain(5, TS('('), NS('U'), TS('>'), NS('U'), TS(')'))
		),
		Rule(NS('L'), GRB_ERROR_SERIES + 8, 	// Ошибка при начальной инициализации
			25,	//vi = l || vi = i || vi = i(i)
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
