...
void LexAnalysis(In::IN& in, LT::LexTable& lex, IT::IdTable& id)
{
	...
	bool lexID = true, lexShort = false, lexComment = false;
	int main = 0;
	...
	while (FindLexema(in, lexema))
	{
		...
		switch (*lexema)
		{
		case LEXSEMA_NOTEQUALS:
		case LEXSEMA_COMPARE:
		case LEXSEMA_RIGHTBRACE:
		case LEXSEMA_SEMICOLON:
		case LEXSEMA_COMMA:
		...
		case 'f':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_FUNCTION, currentRow, currentLex));
				idtype = IT::IDTYPE::F;
				lexID = false;
			}
			break;
		...
		case 'm':
		{
			...
			{
				main++;
				LT::Add(lex, new LT::Entry(LEXSEMA_MAIN, currentRow, currentLex));
		...
		case 'p':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_PARAM, currentRow, currentLex));
				idtype = IT::IDTYPE::P;
		...
		case 'r':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_RETURN, currentRow, currentLex));
				lexID = false;
			}
			if (lexID)
			{
				...
				if (FST::execute(fst_var) && !lexComment)
				{
					...
					static int count = 0;
					sprintf(str, "%d", count++);
					LT::Add(lex, new LT::Entry(LEXSEMA_REPEAT, currentRow, currentLex));
		...
		case 's':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_STR, currentRow, currentLex));
			...
			if (lexID) 
			{
				...
				{
					LT::Add(lex, new LT::Entry(LEXSEMA_SHORT, currentRow, currentLex));
		...
		case 'v':
		{
			...
			if (FST::execute(fst_var) && !lexComment)
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_VAR, currentRow, currentLex));
				idtype = IT::IDTYPE::V;
				lexID = false;
			}
			break;
		...
		case 'c':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_CP, currentRow, currentLex));
				lexID = false;
			}
			break;
		}
		...
		case '+':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_PLUS, *lexema, currentRow, currentLex));
				lexID = false;
			}
			break;
		}
		case '-':
		{
			...
			if (lexID)
			{
				...
				{
					LT::Add(lex, new LT::Entry(LEXSEMA_MINUS, *lexema, currentRow, currentLex));
					lexID = false;
				}
			}
			break;
		...
		case '*':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_TIMES, *lexema, currentRow, currentLex));
				lexID = false;
			}
			break;
		}
		case '/':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_DIVIDE, *lexema, currentRow, currentLex));
				lexID = false;
			}
			break;
		...
		case '%':
		{
			...
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_MODULE, *lexema, currentRow, currentLex));
				lexID = false;
			}
			break;
		}
		...
		case IN_CODE_QUOTES:
		{
			...
			if (FST::execute(fst_literal_s) && !lexComment)
			{
				...
				{
					LT::Add(lex, new LT::Entry(LEXSEMA_LITERAL, currentRow, currentLex, IsLiteralString));
				}
				else
				{
					LT::Add(lex, new LT::Entry(LEXSEMA_LITERAL, currentRow, currentLex, indexIdTable++));
		...
		if (lexShort && !lexComment)
		{
			...
			if (IsLiteralInt + 1)
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_LITERAL, currentRow, currentLex, IsLiteralInt));
			}
			else
			{
				LT::Add(lex, new LT::Entry(LEXSEMA_LITERAL, currentRow, currentLex, indexIdTable++));
			...
			if (FST::execute(fst_id))
			{
				...
				if (isId + 1)
				{
					if (idtype != IT::IDTYPE::D)
					{
						...
						LT::Add(lex, new LT::Entry(LEXSEMA_ID, currentRow, currentLex, indexIdTable++));
						IT::Add(id, new IT::Entry(lex.current_size - 1, areaOfVisibility.top(), lexema, iddatatype, idtype));

						ID_RESET
					}
					else
						LT::Add(lex, new LT::Entry(LEXSEMA_ID, currentRow, currentLex, isId));
				}
				else
				{
					if (iddatatype == IT::IDDATATYPE::DEF)
