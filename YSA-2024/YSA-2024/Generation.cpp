...
namespace GN
{
	...
	void GenCode(LT::LexTable& lextable, IT::IdTable& idtable, ostream* file)
	{
		...
		for (int i = 0; i < lextable.current_size; i++)
		{
			...
			case LEXSEMA_ID: // вызов функции
			{
				if (LT_ENTRY(i + 1)->lexema == LEXSEMA_LEFTHESIS && LT_ENTRY(i + 1)->lexema != LEXSEMA_FUNCTION) // не объ€вление, а вызов
			...
			case LEXSEMA_RIGHTBRACE:	// переход на метку в конце кондишна
			{
				open--;
			...
			case LEXSEMA_REPEAT: // цикл с условием (метка)
			{
				branchingnNum++;
			...
			case LEXSEMA_EQUAL: // присваивание (вычисление выражений)
			{
				PN::polishNotation(i, lextable, idtable);
				str = GenEqualCode(lextable, idtable, i);
				break;
			}
			case LEXSEMA_CP: // вывод
			{
	...
	string GenEqualCode(LT::LexTable& lextable, IT::IdTable& idtable, int& i)
	{
		...
		case IT::IDDATATYPE::STR:	// разрешить присваивать строкам только строки, литералы и вызовы функций
		{
			...
			if (lex == LEXSEMA_ID && (e2->idtype == IT::IDTYPE::F)) // вызов функции
			{
				str += GenCallFuncCode(lextable, idtable, i);
				str += "mov " + string(e1->FullName) + ", eax";
			}
			else if (lex == LEXSEMA_LITERAL) // литерал
			{
	...
	string GenFunctionCode(LT::LexTable& lextable, IT::IdTable& idtable, int& i)
	{
		...
		while (LT_ENTRY(i)->lexema != LEXSEMA_RIGHTHESIS) // пока параметры не кончатс€
		{
			if (LT_ENTRY(i)->lexema == LEXSEMA_ID) // параметр
	...
	string GenExitCode(LT::LexTable& lextable, IT::IdTable& idtable, int& i, string funcname)
	{
		...
		if (LT_ENTRY(i + 1)->lexema != LEXSEMA_SEMICOLON)	// выход из функции (вернуть значение)
		{
	...
	string GenBranchingCode(LT::LexTable& lextable, IT::IdTable& idtable, int& i, int branchingnNum)
	{
		...
		{	//метки переходов в процессе сравнени€
			case LEXSEMA_MORE:  tstr = "jg";  fstr = "jl";  break; //JG - если первый операнд больше второго          jl - если первый операнд меньше второго
			case LEXSEMA_LESS:   tstr = "jl";  fstr = "jg";  break;
			case LEXSEMA_COMPARE: tstr = "jz";  fstr = "jnz";  break; //jz - переход, если равно        jnz - переход, если не равно
