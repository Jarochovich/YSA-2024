...
namespace SM
{
	void semAnaliz(LT::LexTable& lextable, IT::IdTable& idtable)
	{
		for (int i = 0; i < lextable.current_size; i++)
		{
			...
			{
			case LEXSEMA_DIVIDE:	// деление на 0
			{
			...
			case LEXSEMA_MODULE: // деление по модулю на 0
			{
				if (lextable.table[i]->sign == '%' && (lextable.table[i + 1]->lexema == LEXSEMA_LITERAL || lextable.table[i + 1]->lexema == LEXSEMA_ID))
				{
					if (idtable.table[lextable.table[i + 1]->idxTI]->value.vshort == 0)
			...
			case LEXSEMA_EQUAL: // выражение
			{
				if (i)
				{
					...
					{
						// проверка выхода числовых значений за указанный диапазон
						...
						if (lefttype == IT::IDDATATYPE::STR) // справа только литерал, ид или вызов строковой ф-ции
						{
							...
							if (l == LEXSEMA_PLUS || l == LEXSEMA_MINUS || l == LEXSEMA_TIMES || l == LEXSEMA_DIVIDE || l == LEXSEMA_MODULE) // выражени€ недопустимы
			...
			case LEXSEMA_ID: // проверка типа возвращаемого значени€  
			{
				IT::Entry* e;
				e = idtable.table[lextable.table[i]->idxTI];

				if (i && lextable.table[i - 1]->lexema == LEXSEMA_FUNCTION)	// объ€вление функции
				{
					for (int k = i + 1; ; k++)
					{
						char l = lextable.table[k]->lexema;
				...
				if (lextable.table[i + 1]->lexema == LEXSEMA_LEFTHESIS && lextable.table[i - 1]->lexema != LEXSEMA_FUNCTION) // именно вызов
				{
